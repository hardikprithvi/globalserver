...
config['DEFAULT']['email send pass'] = decrypt_message(bytes(config['DEFAULT']['email send pass'],'utf-8'))
...
def raiseticket(emailid,issue,t_id):
	...
	s = smtplib.SMTP(config["DEFAULT"]["email send server"])
	s.starttls()
...
def updateticket(emailid,issue,assign,t_id):
	email_content2 = """
	<!DOCTYPE html>
		<head>
    <meta charset="utf-8"> <!-- utf-8 works for most cases -->
